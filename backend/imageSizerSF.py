...
template = cv2.imread('backend/assets/WON.png', cv2.IMREAD_GRAYSCALE)
# w, h = template.shape[::-1]
# template = cv2.resize(img,(20, 50), fx=1, fy=1)


def Winner():
    img = cv2.imread('backend/assets/WON.png', 0)
    template = cv2.imread('backend/assets/WON.png', 0)
    w, h = template.shape[::-1]
    print(img)
...
cap = cv2.VideoCapture('backend/video/Streetfighter6test.mp4', 0)
